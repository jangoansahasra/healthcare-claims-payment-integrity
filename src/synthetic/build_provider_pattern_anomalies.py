...
from src.synthetic.build_ledger_temporal_anomalies import (
    GROUND_TRUTH_TABLES,
    build_ledger_temporal_anomalies,
    read_rows,
)
...
DEFAULT_QUALITY_REPORT_PATH = Path(
    "data/metadata/quality/provider_pattern_anomaly_injection.json"
)


def build_provider_pattern_anomalies(
    ...
    """Rebuild prior stages and append deterministic PI007 and PI008."""
    ...
    build_ledger_temporal_anomalies(
        ...
        record_level_report_path=output_root / "_record_stage_quality.json",
        quality_report_path=output_root / "_ledger_stage_quality.json",
    )
    ...
    extended, injections, changes = inject_provider_pattern_anomalies(
        before_stage,
        ground_truth["anomaly_injection"],
        ground_truth["anomaly_field_change"],
        anomaly_contract,
    )
    for name in PROVIDER_CHANGED_TABLES:
        write_parquet_atomic(
            extended[name],
    ...
    checks = validate_provider_pattern_anomalies(
    ...
    new_injections = [
        row for row in injections if row["rule_id"] in PROVIDER_PATTERN_RULES
    ]
    ...
    write_sample_csv(
        ...
        sample_root / "provider_pattern_anomaly_injection_sample.csv",
    )
    write_sample_csv(
        new_changes,
        contract_table_schema(anomaly_contract, "anomaly_field_change").names,
        sample_root / "provider_pattern_field_change_sample.csv",
    )
    ...
    if not report["all_checks_passed"]:
        ...
        raise SyntheticAnomalyError(
            "Provider-pattern anomaly checks failed: " + ", ".join(failed)
        )
    return report
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Inject deterministic provider amount and utilization anomalies."
    )
    parser.add_argument(
...
def main() -> None:
    args = parse_args()
    report = build_provider_pattern_anomalies(
