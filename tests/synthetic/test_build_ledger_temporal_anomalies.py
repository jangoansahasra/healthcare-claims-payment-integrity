...
def test_composed_ledger_stage_is_reproducible(tmp_path: Path) -> None:
    ...
    arguments = {
        ...
        "record_level_report_path": tmp_path / "record_quality.json",
        "quality_report_path": tmp_path / "ledger_quality.json",
    }
    first = build_ledger_temporal_anomalies(**arguments)
    output_names = (
        "payment_transaction",
        "anomaly_injection",
    ...
    repeated = build_ledger_temporal_anomalies(**arguments)

    assert first["all_checks_passed"] is True
    assert repeated["all_checks_passed"] is True
    assert first["injection_count"] == 300
    assert first["ground_truth"] == repeated["ground_truth"]
    assert first["payment_transaction"] == repeated["payment_transaction"]
    assert first_bytes == {
        name: (output_root / f"{name}.parquet").read_bytes() for name in output_names
    }
