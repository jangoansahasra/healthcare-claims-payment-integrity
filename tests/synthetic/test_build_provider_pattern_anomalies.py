...
def test_composed_provider_pattern_stage_is_reproducible(tmp_path: Path) -> None:
    ...
    arguments = {
        ...
        "quality_report_path": tmp_path / "provider_quality.json",
    }
    first = build_provider_pattern_anomalies(**arguments)
    output_names = (
        "claim_header",
        "claim_line",
        "adjudication_event",
    ...
    repeated = build_provider_pattern_anomalies(**arguments)

    assert first["all_checks_passed"] is True
    assert repeated["all_checks_passed"] is True
    assert first["injection_count"] == 400
